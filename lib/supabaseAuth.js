...
/** Force DNS to resolve to IPv4 — Railway containers have no IPv6 */
function getIPv4ConnectionString(url) {
    try {
        const parsed = new URL(url);
        const hostname = parsed.hostname;
        // Replace hostname with 127.0.0.1 placeholder first
        const ipv4Url = url.replace(hostname, 'IPV4_PLACEHOLDER');
        return new Promise((resolve, reject) => {
            // Only resolve IPv4 (A record), skip IPv6 (AAAA)
            dns.resolve4(hostname, (err, addresses) => {
                if (err) {
                    console.error('[SupabaseAuth] DNS IPv4 resolution failed for', hostname, err.message);
                    reject(err);
                    return;
                }
                const ipv4 = addresses[0];
                console.log('[SupabaseAuth] DNS resolved', hostname, '->', ipv4, '(IPv4 forced)');
                resolve(ipv4Url.replace('IPV4_PLACEHOLDER', ipv4));
            });
        });
    } catch (e) {
        console.error('[SupabaseAuth] Failed to parse DATABASE_URL:', e.message);
...
async function getDb() {
    ...
    dbConnecting = true;
    try {
        // Force IPv4 — Railway containers have NO IPv6 connectivity
        const connStr = await getIPv4ConnectionString(DATABASE_URL);
        console.log('[SupabaseAuth] Connecting to database (IPv4)...');
