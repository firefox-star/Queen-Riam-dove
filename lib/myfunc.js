...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    // ── Plugin System Hook ──
    // Check if this message matches any installed plugin command.
    // Non-blocking, error-safe — never breaks the bot.
    try {
        const { checkPluginCommand } = require('./pluginLoader');
        checkPluginCommand(XeonBotInc, m);
    } catch (e) { /* silent — plugin system not loaded yet or error */ }
